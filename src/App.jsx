...
const detectFranchise = (title) => {
    ...
    if (t.includes('final fantasy')) return 'Final Fantasy';
    if (t.includes('kingdom hearts')) return 'Kingdom Hearts';
    if (t.includes('katamari')) return 'Katamari';
    if (t.includes('mario') || t.includes('luigi') || t.includes('yoshi') || t.includes('peach') || t.includes('princesa') || t.includes('toad')) return 'Super Mario';
    if (t.includes('sonic')) return 'Sonic The Hedgehog';
    if (t.includes('last of us')) return 'The Last Of Us';
    if (t.includes('zelda') || t.includes('link')) return 'Zelda';
    if (t.includes('spyro')) return 'Spyro';
    if (t.includes('rayman')) return 'Rayman';
    if (t.includes('megaman') || t.includes('mega man') || t.includes('rockman') || t.includes('tron bonne')) return 'Megaman';
    if (t.includes('dragon ball') || t.includes('dbz')) return 'Dragon Ball';
    if (t.includes('crash bandicoot') || t.includes('crash team racing') || /\bcrash\b/.test(t)) return 'Crash Bandicoot';
    if (t.includes('castlevania') || t.includes('bloodstained')) return 'Castlevania';
    if (t.includes('persona') || t.includes('shin megami tensei')) return 'Persona';
    if (t.includes('tomb raider') || t.includes('lara croft')) return 'Tomb Raider';
    if (t.includes('xeno')) return 'Xenoblade'; 
    if (t.includes('wario')) return 'Wario Ware';
    if (t.includes('devil may cry') || t.includes('dmc')) return 'Devil May Cry';
    if (t.includes('dragon quest')) return 'Dragon Quest';
    if (t.includes('fallout')) return 'Fallout';
    if (t.includes('grand theft auto') || t.includes('gta')) return 'GTA';
    if (t.includes('resident evil') || t.includes('biohazard')) return 'Resident Evil';
...
export default function App() {
  ...
  const [topSorts, setTopSorts] = useState({
    ...
    franquiasNota: { key: 'avgNota', dir: 'desc' },
    franquiasTempo: { key: 'totalTempo', dir: 'desc' },
  ...
  const dashboardStats = useMemo(() => {
    ...
    let stats = { 
      ...
      dif: { A: 0, B: 0, C: 0, D: 0, E: 0 }, consoles: {}, generos: {}, anos: {}, franquias: {}, midiaCategoria: {}, midiaSub: {},
    ...
    baseList.forEach(g => {
      ...
      if (g.status === 'Finalizado') {
          ...
          let franchiseName = detectFranchise(getVal(g, ['titulo', 'nome']));
          if (franchiseName) {
            if(!stats.franquias[franchiseName]) stats.franquias[franchiseName] = { count: 0, totalNota: 0, notaCount: 0, totalTempo: 0 };
            stats.franquias[franchiseName].count++;
            stats.franquias[franchiseName].totalTempo += tHrs;
            if(!isNaN(n)) { stats.franquias[franchiseName].totalNota += n; stats.franquias[franchiseName].notaCount++; }
          }
      }
    });
    ...
    Object.keys(stats.franquias).forEach(c => stats.franquias[c].avgNota = stats.franquias[c].notaCount > 0 ? (stats.franquias[c].totalNota / stats.franquias[c].notaCount).toFixed(1) : 0);
    return stats;
          ...
          {activeTab === 'dashboard' && (
             <div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 items-start">
                  ...
                  <PieChartUI title="Top Franquias" slices={getGenericPieSlices(dashboardStats.franquias, 'franquia')} onClickSlice={(s) => s.type && setViewModal({type: s.type, data: s.data})} />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 items-start">
                   ...
                   {}
                   <div className={`p-4 bg-white ${theme.border} shadow-[4px_4px_0_0_rgba(15,23,42,1)] overflow-x-auto h-fit`}>
                     <h3 className="text-[10px] font-black uppercase mb-4 border-b-[3px] border-slate-900 pb-1">Top Franquias (Nota)</h3>
                     <div className="max-h-[250px] overflow-y-auto custom-scrollbar pr-1">
                       <table className="w-full text-left text-[10px] font-black uppercase">
                         <thead className="sticky top-0 bg-white z-10 shadow-sm"><tr className="border-b-[2px] border-slate-900"><MiniTh label="Franquia" sortKey="name" tableKey="franquiasNota"/><MiniTh label="Jogos" sortKey="count" tableKey="franquiasNota" align="center"/><MiniTh label="Nota" sortKey="avgNota" tableKey="franquiasNota" align="center"/></tr></thead>
                         <tbody>
                           {Object.keys(dashboardStats.franquias).map(c => ({ name: c, ...dashboardStats.franquias[c] })).sort((a,b) => {
                              let valA = a[topSorts.franquiasNota.key]; let valB = b[topSorts.franquiasNota.key];
                              if (topSorts.franquiasNota.key === 'name') return topSorts.franquiasNota.dir === 'asc' ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name);
                              return topSorts.franquiasNota.dir === 'asc' ? valA - valB : valB - valA;
                           }).map(stat => (
                               <tr key={stat.name} className="border-b border-slate-200 hover:bg-slate-50">
                                 <td className="py-2"><span onClick={()=>setViewModal({type:'franquia', data: stat.name})} className="inline-block cursor-pointer px-1.5 py-0.5 border-[2px] border-slate-900 shadow-[1px_1px_0_0_rgba(15,23,42,1)] bg-pink-100 hover:-translate-y-0.5 transition-transform">{stat.name}</span></td>
                                 <td className="py-2 text-center">{stat.count}</td>
                                 <td className="py-2 text-center">{stat.avgNota}</td>
                               </tr>
                           ))}
                   ...
                   <div className={`p-4 bg-white ${theme.border} shadow-[4px_4px_0_0_rgba(15,23,42,1)] overflow-x-auto h-fit`}>
                     <h3 className="text-[10px] font-black uppercase mb-4 border-b-[3px] border-slate-900 pb-1">Top Franquias (Tempo)</h3>
                     <div className="max-h-[250px] overflow-y-auto custom-scrollbar pr-1">
                       <table className="w-full text-left text-[10px] font-black uppercase">
                         <thead className="sticky top-0 bg-white z-10 shadow-sm"><tr className="border-b-[2px] border-slate-900"><MiniTh label="Franquia" sortKey="name" tableKey="franquiasTempo"/><MiniTh label="Jogos" sortKey="count" tableKey="franquiasTempo" align="center"/><MiniTh label="Tempo" sortKey="totalTempo" tableKey="franquiasTempo" align="center"/></tr></thead>
                         <tbody>
                           {Object.keys(dashboardStats.franquias).map(c => ({ name: c, ...dashboardStats.franquias[c] })).sort((a,b) => {
                              let valA = a[topSorts.franquiasTempo.key]; let valB = b[topSorts.franquiasTempo.key];
                              if (topSorts.franquiasTempo.key === 'name') return topSorts.franquiasTempo.dir === 'asc' ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name);
                              return topSorts.franquiasTempo.dir === 'asc' ? valA - valB : valB - valA;
                           }).map(stat => (
                               <tr key={stat.name} className="border-b border-slate-200 hover:bg-slate-50">
                                 <td className="py-2"><span onClick={()=>setViewModal({type:'franquia', data: stat.name})} className="inline-block cursor-pointer px-1.5 py-0.5 border-[2px] border-slate-900 shadow-[1px_1px_0_0_rgba(15,23,42,1)] bg-pink-100 hover:-translate-y-0.5 transition-transform">{stat.name}</span></td>
                                 <td className="py-2 text-center">{stat.count}</td>
          ...
          <div className={`w-full max-w-[1400px] bg-white ${theme.border} ${theme.card} flex flex-col my-auto shadow-2xl`}>
            ...
            <div className={`p-4 border-b-[3px] border-slate-900 flex justify-between items-center ${viewModal.type === 'game' && isEditingFicha ? theme.pink : theme.cyan}`}>
              <h2 className="text-xl sm:text-2xl font-black uppercase tracking-tighter truncate pr-4">
                ...
                {viewModal.type === 'franquia' && `Ficha da Franquia: ${viewModal.data}`}
            ...
            <div className="p-4 sm:p-6 bg-slate-50 overflow-y-auto max-h-[75vh] custom-scrollbar">
              ...
              {viewModal.type !== 'game' && (() => {
                 let filteredList = games.filter(g => {
                    ...
                    if (viewModal.type === 'franquia') return g.status === 'Finalizado' && detectFranchise(getVal(g, ['titulo', 'nome'])) === viewModal.data;
